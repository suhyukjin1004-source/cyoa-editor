...
- **B. IndexedDB 저장 전환** (ICCPlus): 현재 localStorage(~5MB) 한계로 이미지 많은 대형 프로젝트에서 저장 실패. 다중 슬롯(99개)·오토세이브도 함께. **가장 우선순위 높은 후속.**
- **C. 외부/커스텀 폰트(@font-face URL)** (ICCPlus): 설정에서 웹폰트 URL을 받아 주입. 저비용이나 외부 URL 로드 보안면 고려 필요.
